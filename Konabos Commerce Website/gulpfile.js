...
gulp.task('03-Publish-Feature-To-Website', function (callback) {
    return runSequence(
        "Stop-Local-IIS",
        "Build-Feature-Compare",
...
gulp.task('01-Build-Feature', function (callback) {
    var targets = ["Build"];
    if (config.runCleanBuilds) {
        targets = ["Clean", "Build"];
    }

    var solution = "./" + config.featureProjectPath + '/Feature.Konabos.Loyalty.Website.csproj';
...
gulp.task("02-Publish-Feature-Projects", function () {
    publishProjects("./2. Feature/Feature.Loyalty/Website/");
});
